...
(function (app) {
    ...
    function ensurePMData(cb) {
        ...
        }, function () { app.cachedData.pmEmployees = []; done.e = []; if (done.p !== null) cb(done.e, done.p); });
        app.fetchJson('/api/projects', function (data) {
            app.cachedData.pmProjects = data;
            done.p = data;
            if (done.e !== null) cb(done.e, done.p);
        }, function () { app.cachedData.pmProjects = []; done.p = []; if (done.e !== null) cb(done.e, done.p); });
    }
    ...
    function findTeamMemberIds(projects) {
        ...
        projects.forEach(function (p) {
            (p.teamMemberIds || []).forEach(function (id) { if (id) ids[id] = true; });
        });
        return Object.keys(ids);
    }

    function findTeamMembers(employees, projects) {
        var memberIds = findTeamMemberIds(projects);
        return employees.filter(function (e) {
            return memberIds.indexOf(e.id) !== -1;
        });
    }
        ...
        ensurePMData(function (employees, projects) {
            ...
            if (projFilter) {
                ...
                managed.forEach(function (p) {
                    projFilter.innerHTML += '<option value="' + app.esc(p.id) + '">' + app.esc(p.name || 'Untitled') + '</option>';
                });
            }
            ...
            if (assignFilter) {
                ...
                team.forEach(function (e) {
                    ...
                    assignFilter.innerHTML += '<option value="' + app.esc(e.id) + '">' + app.esc(name) + '</option>';
                });
            }
            ...
            if (projSelect) {
                ...
                managed.forEach(function (p) {
                    projSelect.innerHTML += '<option value="' + app.esc(p.id) + '" data-name="' + app.esc(p.name || '') + '">' + app.esc(p.name || 'Untitled') + '</option>';
                });
            }
            ...
            if (assignSelect) {
                ...
                team.forEach(function (e) {
                    var name = app.buildName(e);
                    assignSelect.innerHTML += '<option value="' + app.esc(e.id) + '" data-name="' + app.esc(name) + '">' + app.esc(name) + '</option>';
                });
            }

            app.fetchJson('/api/workspace/tasks', function (tasks) {
                var managedIds = {};
                managed.forEach(function (p) { managedIds[p.id] = true; });
                pmTasksCache = tasks.filter(function (t) { return managedIds[t.projectId]; });
    ...
    function renderPMTaskBoard(tasks) {
        var todo = [], inProgress = [], underReview = [], completed = [];
        ...
        if (statsEl) {
            statsEl.innerHTML =
                pmStatCard(tasks.length, 'Total Tasks', 'Across managed projects', 'blue') +
                pmStatCard(inProgress.length, 'In Progress', 'Currently active', 'green') +
                pmStatCard(underReview.length, 'Under Review', 'Awaiting review', 'orange') +
        ...
        renderTaskCards('cardsTaskUnderReview', underReview, 'No tasks under review');
    ...
    function renderTaskCard(t) {
        ...
        var isOverdue = t.dueDate && t.dueDate < Date.now() && (t.status || '').toUpperCase() !== 'COMPLETED';
        return '<div class="task-card clickable" onclick="openTaskModal(\'' + id + '\')">'
            + '<div class="task-card-header"><span class="task-card-title">' + app.esc(t.title || 'Untitled') + '</span>'
            + '<span class="card-priority ' + priorityClass + '">' + app.esc(app.formatStatus(t.priority || 'MEDIUM')) + '</span></div>'
            + (t.description ? '<div class="task-card-desc">' + app.esc(t.description.length > 100 ? t.description.substring(0, 100) + '...' : t.description) + '</div>' : '')
            + '<div class="task-card-meta">'
            + '<span class="task-card-project">' + app.esc(t.projectName || 'No project') + '</span>'
            + (dueText ? '<span class="task-card-due' + (isOverdue ? ' overdue' : '') + '">' + dueText + '</span>' : '')
            + '</div>'
            + (t.assignedToName ? '<div class="task-card-assignee"><div class="mini-avatar">' + app.initials(t.assignedToName) + '</div><span>' + app.esc(t.assignedToName) + '</span></div>' : '<div class="task-card-assignee unassigned">Unassigned</div>')
            + '</div>';
    }

    window.filterPMTasks = function () {
        var projVal = (document.getElementById('taskProjectFilter') || {}).value || '';
        var assignVal = (document.getElementById('taskAssigneeFilter') || {}).value || '';
    ...
    window.openTaskModal = function (id) {
        ...
        document.getElementById('taskDeleteBtn').classList.toggle('hidden', !isEdit);

        if (isEdit) {
            var task = null;
            for (var i = 0; i < pmTasksCache.length; i++) {
                if (pmTasksCache[i].id === id) { task = pmTasksCache[i]; break; }
            }
            if (task) {
                ...
                document.getElementById('taskDueDate').value = task.dueDate ? app.toDateInput(task.dueDate) : '';
            }
        } else {
            ['taskId', 'taskTitle', 'taskDescription', 'taskDueDate'].forEach(function (fid) {
    ...
    window.savePMTask = function () {
        ...
        app.fetchMutate(method, url, payload, function () {
            ...
            // Activity log
            logActivity(isEdit ? 'UPDATE' : 'CREATE', 'task', id || '', payload.title, payload.projectId, isEdit ? 'Task updated' : 'Task created: ' + payload.title);
            // Notify assigned employee
            if (payload.assignedTo) {
                app.fetchMutate('POST', '/api/workspace/notifications', {
                    recipientId: payload.assignedTo,
                    type: 'TASK_ASSIGNED',
                    message: (isEdit ? 'Task updated: ' : 'New task assigned: ') + payload.title,
                    entityId: id || '',
                    entityType: 'task',
                    link: '/workspace'
                }, function () {}, function () {});
    ...
    window.deletePMTask = function () {
        ...
        app.showConfirm('Delete this task?', function () {
            app.fetchMutate('DELETE', '/api/workspace/tasks/' + encodeURIComponent(id), null, function () {
                app.closeModal('taskModal');
                ...
                app.showModalNotice('taskModalNotice', err || 'Failed to delete task.', 'error');
            });
        });
    };
    ...
    window.postTaskComment = function () {
        ...
        app.fetchMutate('POST', '/api/workspace/comments', { taskId: taskId, text: text }, function () {
            ...
            var task = null;
            for (var i = 0; i < pmTasksCache.length; i++) {
                if (pmTasksCache[i].id === taskId) { task = pmTasksCache[i]; break; }
            }
            logActivity('COMMENT', 'task', taskId, task ? task.title : '', task ? task.projectId : '', 'Added a comment');
        }, function (err) {
            app.showToast(err || 'Failed to post comment', 'error');
        });
    };

    function logActivity(action, entityType, entityId, entityName, projectId, details) {
    ...
    window.loadPMProjects = function () {
        ensurePMData(function (employees, projects) {
            ...
            document.getElementById('pmProjectsStats').innerHTML =
                pmStatCard(managed.length, 'Total Projects', 'Projects you manage', 'blue') +
            ...
            var html = '';
            if (managed.length === 0) {
                html = '<div class="empty-state">No managed projects. Projects assigned to you as PM will appear here.</div>';
            } else {
                managed.forEach(function (p) {
                    var pct = p.completionPercentage || 0;
                    var badge = isPMCompleted(p) ? 'green' : isPMOverdue(p) ? 'orange' : 'blue';
                    var teamCount = (p.teamMemberIds || []).length;
                    var pid = app.esc(p.id || '');
                    html += '<div class="emp-project-item clickable" onclick="openPMProjectDetail(\'' + pid + '\')">'
                        ...
                        + '<div class="emp-project-meta"><span>Progress: ' + pct + '%</span><span>' + pmDueText(p.endDate) + '</span>'
                        + '<span>Team: ' + teamCount + ' member' + (teamCount === 1 ? '' : 's') + '</span>'
    ...
    window.openPMCreateProject = function () {
        ...
        teamEl.innerHTML = '<span class="comment-empty">Loading employees...</span>';

        ensurePMData(function (employees) {
            if (!employees || employees.length === 0) {
                teamEl.innerHTML = '<span class="comment-empty">No employees found.</span>';
            } else {
                var html = '';
                employees.forEach(function (e) {
                    var name = app.buildName(e);
                    var dept = e.department || 'Unassigned';
                    html += '<label class="team-checkbox-item">'
                        + '<input type="checkbox" value="' + app.esc(e.id) + '" data-name="' + app.esc(name) + '">'
                        + '<span>' + app.esc(name) + '</span>'
                        + '<span class="team-checkbox-dept">' + app.esc(dept) + '</span>'
                        + '</label>';
                });
    ...
    window.savePMProject = function () {
        ...
        var teamIds = [];
        var teamNames = [];
        var checkboxes = document.querySelectorAll('#pmProjTeamCheckboxes input[type="checkbox"]:checked');
        ...
        app.fetchMutate('POST', '/api/projects', payload, function () {
            ...
            logActivity('CREATE', 'project', '', payload.name, '', 'PM created project: ' + payload.name);
            delete app.cachedData.pmProjects;
            loadPMProjects();
        }, function (err) {
            if (btn) btn.disabled = false;
            app.showModalNotice('pmProjNotice', err || 'Failed to create project.', 'error');
        });
    };
    ...
    window.openPMProjectDetail = function (projectId) {
        ensurePMData(function (employees, projects) {
            ...
            document.getElementById('pmProjectDetailTitle').textContent = project.name || 'Project Details';
            ...
            document.getElementById('pmProjectDetailInfo').innerHTML =
                ...
                + '<div class="detail-row"><span class="detail-label">Status</span><span class="badge badge-muted">' + app.esc(app.formatStatus(project.status || '')) + '</span></div>'
                + '<div class="detail-row"><span class="detail-label">Priority</span><span class="card-priority priority-' + (project.priority || 'MEDIUM').toLowerCase() + '">' + app.esc(project.priority || 'N/A') + '</span></div>'
                + '<div class="detail-row"><span class="detail-label">Department</span><span>' + app.esc(project.department || 'None') + '</span></div>'
                + '<div class="detail-row"><span class="detail-label">Team</span><span>' + teamCount + ' member' + (teamCount === 1 ? '' : 's') + '</span></div>'
                + '<div class="detail-row"><span class="detail-label">Progress</span><div class="project-progress" style="flex:1;margin-left:8px"><div class="project-progress-fill" style="width:' + pct + '%"></div></div><span style="margin-left:6px">' + pct + '%</span></div>'
                + '<div class="detail-row"><span class="detail-label">Due</span><span>' + (project.endDate ? app.formatTimestamp(project.endDate) : 'No deadline') + '</span></div>'
                + (project.description ? '<div class="detail-row full-width"><span class="detail-label">Description</span><p>' + app.esc(project.description) + '</p></div>' : '')
                + '</div>';
            loadMilestones(projectId);
            document.getElementById('pmProjectDetailModal').classList.remove('hidden');
        });
    };
    ...
    function loadMilestones(projectId) {
        ...
        if (!list) return;
        list.innerHTML = '<div class="loading-spinner visible"></div>';
        app.fetchJson('/api/workspace/milestones?projectId=' + encodeURIComponent(projectId), function (milestones) {
            if (!milestones || milestones.length === 0) {
                list.innerHTML = '<div class="comment-empty">No milestones yet. Add milestones to track project phases.</div>';
                return;
            }
            list.innerHTML = milestones.map(function (m) {
                var statusClass = m.status === 'COMPLETED' ? 'green' : m.status === 'IN_PROGRESS' ? 'blue' : 'muted';
                var dueText = m.dueDate ? app.formatTimestamp(m.dueDate) : 'No due date';
                var isOverdue = m.dueDate && m.dueDate < Date.now() && m.status !== 'COMPLETED';
                var mid = app.esc(m.id || '');
                return '<div class="milestone-item">'
                    + '<div class="milestone-item-header">'
                    + '<span class="milestone-title">' + app.esc(m.title) + '</span>'
                    + '<span class="badge badge-' + statusClass + '">' + app.esc(app.formatStatus(m.status || 'PENDING')) + '</span>'
                    + '</div>'
                    + (m.description ? '<div class="milestone-desc">' + app.esc(m.description) + '</div>' : '')
                    + '<div class="milestone-meta">'
                    + '<span class="' + (isOverdue ? 'overdue' : '') + '">Due: ' + dueText + '</span>'
                    + '<button class="btn-tiny" onclick="editMilestone(\'' + mid + '\')">Edit</button>'
                    + '<button class="btn-tiny btn-danger" onclick="deleteMilestone(\'' + mid + '\')">Delete</button>'
                    + '</div></div>';
            }).join('');
        }, function () {
            list.innerHTML = '<div class="comment-empty">Failed to load milestones.</div>';
        });
    }
    ...
    window.editMilestone = function (milestoneId) {
        ...
        app.fetchJson('/api/workspace/milestones?projectId=' + encodeURIComponent(pmProjectDetailCache.id), function (milestones) {
            var m = null;
            for (var i = 0; i < milestones.length; i++) {
                if (milestones[i].id === milestoneId) { m = milestones[i]; break; }
            }
            if (!m) return;
            document.getElementById('milestoneForm').classList.remove('hidden');
            document.getElementById('milestoneTitle').value = m.title || '';
            document.getElementById('milestoneDesc').value = m.description || '';
            document.getElementById('milestoneDueDate').value = m.dueDate ? app.toDateInput(m.dueDate) : '';
            document.getElementById('milestoneStatus').value = m.status || 'PENDING';
    ...
    window.saveMilestone = function () {
        ...
        var isEdit = !!mid;
        var url = isEdit ? '/api/workspace/milestones/' + encodeURIComponent(mid) : '/api/workspace/milestones';
        var method = isEdit ? 'PUT' : 'POST';
        app.fetchMutate(method, url, payload, function () {
            app.showToast(isEdit ? 'Milestone updated' : 'Milestone created', 'success');
            cancelMilestoneForm();
            loadMilestones(pmProjectDetailCache.id);
            logActivity(isEdit ? 'UPDATE' : 'CREATE', 'milestone', mid || '', title, pmProjectDetailCache.id, (isEdit ? 'Updated' : 'Created') + ' milestone: ' + title);
        }, function (err) {
    ...
    window.deleteMilestone = function (milestoneId) {
        ...
        app.showConfirm('Delete this milestone?', function () {
            app.fetchMutate('DELETE', '/api/workspace/milestones/' + encodeURIComponent(milestoneId), null, function () {
    ...
    window.loadPMTeam = function () {
        ...
        ensurePMData(function (employees, projects) {
            ...
            document.getElementById('pmTeamStats').innerHTML =
                pmStatCard(managed.length, 'Projects', 'Under your management', 'blue') +
                pmStatCard(team.length, 'Team Members', 'Across all projects', 'green') +
                pmStatCard(deptCount, 'Departments', 'Team org groups', 'purple');

            var html = '';
            if (team.length === 0) {
                html = '<div class="empty-state">No team members found. Team members from your projects will appear here.</div>';
            } else {
                if (noDept.length > 0) {
    ...
    function renderPMTeamDeptGroup(deptName, members, isUnassigned) {
        ...
        var rows = members.map(function (e) {
            var name = app.buildName(e);
            var setDeptBtn = isUnassigned ? ' <button class="btn-set-dept-sm" onclick="openQuickDeptModal(\'' + app.esc(e.id || '') + '\',\'' + app.esc(name).replace(/'/g, '') + '\')">Set Dept</button>' : '';
            return '<div class="emp-team-row">'
                + '<div class="emp-team-avatar pm-avatar">' + app.initials(name) + '</div>'
    ...
    function renderPMTimesheetUI(entries, team, managed) {
        var totalHours = entries.reduce(function (s, e) { return s + (e.hours || 0); }, 0);
        var uniqueEmployees = {};
        entries.forEach(function (e) { if (e.employeeId) uniqueEmployees[e.employeeId] = true; });

        document.getElementById('pmTimesheetStats').innerHTML =
            pmStatCard(totalHours.toFixed(1) + 'h', 'Total Hours', 'Across all team members', 'blue') +
            pmStatCard(Object.keys(uniqueEmployees).length, 'Contributors', 'Employees logging time', 'green') +
            pmStatCard(entries.length, 'Entries', 'Total time entries', 'purple');

        var html = '';
        if (entries.length === 0) {
            html = '<div class="empty-state">No timesheet entries from your team yet.</div>';
        } else {
        ...
        if (team.length === 0) {
            ...
            team.slice(0, 8).forEach(function (member) {
                var memberHours = entries
                    .filter(function (e) { return e.employeeId === member.id; })
                    .reduce(function (s, e) { return s + (e.hours || 0); }, 0);
                var name = app.buildName(member);
                summaryHtml += pmInfoRow(name, memberHours.toFixed(1) + 'h logged', member.position || 'Employee');
            });
        }
    ...
    function renderPMDocumentsUI(docs, managed) {
        ...
        document.getElementById('pmDocsStats').innerHTML =
            ...
            pmStatCard(templates.length + briefs.length, 'Resources', 'Templates & briefs', 'purple');
        ...
        if (companyDocs.length === 0) {
            ...
            companyDocs.forEach(function (d) {
                html += pmInfoRow(d.name || 'Untitled', d.description || 'No description', d.category);
            });
        }
        ...
        if (briefs.length === 0 && managed.length === 0) {
            ...
            briefs.forEach(function (d) {
                projHtml += pmInfoRow(d.name || 'Untitled', d.description || 'No description', 'BRIEF');
            });
            managed.slice(0, 5).forEach(function (p) {
                projHtml += pmInfoRow('Brief: ' + (p.name || 'Untitled'),
        ...
        ensurePMData(function (employees) {
            ...
            if (!me) {
                document.getElementById('pmProfileStats').innerHTML =
                    pmStatCard('Unavailable', 'Profile', 'No employee record matched your account', 'purple');
            ...
            document.getElementById('pmProfileStats').innerHTML =
                pmStatCard(me.department || 'N/A', 'Department', 'Assigned department', 'blue') +
                pmStatCard(me.position || 'N/A', 'Position', 'Current role', 'green') +
                pmStatCard(me.status || 'N/A', 'Status', 'Employment status', 'purple');

            var statusClass = (me.status || '').toUpperCase() === 'ACTIVE' ? 'badge-green' : 'badge-orange';
            document.getElementById('pmProfileContent').innerHTML =
                '<div class="emp-profile-header">'
                + '<div class="emp-profile-avatar pm-avatar">' + app.initials(name) + '</div>'
                ...
                + pmProfileRow('Employee ID', me.id || 'N/A')
                + pmProfileRow('Hire Date', me.hireDate ? app.formatTimestamp(me.hireDate) : 'N/A')
                + '</div>';
        });
    };
