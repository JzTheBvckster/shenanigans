...
public class AuthService {
  ...
  private void createEmployeeRecord(User user) {
    try {
      ...
      firestore.collection("employees").document(user.getUid()).set(employeeData).get();
  ...
  public Task<Void> updateUserProfile(User user) {
    return new Task<>() {
      ...
      protected Void call() throws Exception {
        ...
        firestore.collection(USERS_COLLECTION).document(user.getUid()).update(updates).get();
  ...
  private void checkAuthClient() throws FirebaseAuthException {
    if (authClient == null) {
      throw new FirebaseAuthException(
          "Firebase not configured. Please add your API key to firebase.properties");
    }
  }
